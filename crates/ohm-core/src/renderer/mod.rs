//! Contains the [`Renderer`] trait and some utilities for implementing custom renderers.
...
///
/// A renderer should manage resources like surfaces (windows) and textures, and implement the
/// actual rendering of [`DrawList`] onto the surface.
pub trait Renderer: Send + Sync + 'static {
    /// Creates a surface from the specified [`WindowHandle`] with a provided initial size.
    ...
    /// # Panics
    ///
    /// This method is allowed to panic if the provided [`SurfaceId`] is invalid.
    ...
    /// Updates internally managed textures, applying each [`TextureCommand`] in sequence.
    ///
    /// This includes: creating new textures, copying data between textures, writing data to textures,
    /// and destroying textures.
    ///
...
/// A trait for window handles. In most cases, this will be a `Window` from `winit`.
